    ...
    if (pkt.stream_index == video_stream_idx) {
        ...
        if (*got_frame) {
			...
			*/
			...
			size_t unpadded_linesize = frame->nb_samples * bytes_per_sample;
            printf("audio_frame%s n:%d nb_samples:%d bytes_per_sample : %d pts:%s\n",
                   cached ? "(cached)" : "",
                   audio_frame_count++, frame->nb_samples, bytes_per_sample,
...
int demux(int argc, const char **argv)
{
    int ret = 0, got_frame;
